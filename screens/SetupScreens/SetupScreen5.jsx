...
export default function SetupScreen5({ navigation }) {
  const [height, setHeight] = useState(165);

  const handleNext = async () => {
    await AsyncStorage.setItem('height', String(height));
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        {/* 타이틀 */}
        <Text style={styles.title}>키를 선택해주세요</Text>
        ...
        <View style={styles.pickerCard}>
          <Picker
            ...
            onValueChange={(itemValue) => setHeight(Number(itemValue))}
            style={styles.picker}
            itemStyle={styles.pickerItem}
          >
            {Array.from({ length: 121 }, (_, i) => (
              <Picker.Item key={i} label={`${i + 130}`} value={`${i + 130}`} />
            ))}
          </Picker>
        </View>
...
const styles = StyleSheet.create({
  ...
  heightText: { fontSize: 64, fontWeight: "bold", color: "#232222", marginRight: 6 },
